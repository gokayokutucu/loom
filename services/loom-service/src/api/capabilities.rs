use crate::{
    ...
    providers::{
        ...
        gemini::{GeminiRuntime, GeminiSecret},
...
pub async fn discover_models(
    ...
    for profile in profiles {
        ...
        match profile.provider_kind {
            ...
            crate::providers::config::ProviderKind::OpenAi => {
                let mut secret = None;
                if profile.requires_secret {
                    let secret_ref = profile
                        .secret_ref
            ...
            crate::providers::config::ProviderKind::Gemini => {
                let mut secret = None;
                if profile.requires_secret {
                    let secret_ref = profile
                        .secret_ref
                ...
                let runtime = GeminiRuntime::new(profile.clone(), secret);
                match runtime.models().await {
                    Ok(response) => {
                        discovered.extend(
                            ingest_provider_model_names(&repo, &profile, response.models, persist)
