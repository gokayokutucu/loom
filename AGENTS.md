...
## 17) Agent Project Management (Agent-PM) Protocol
...
### Versioning & Tracking Protocols

Whenever you generate a plan, execute a task, or run a test, you MUST adhere to the following file management rules:

- **Plans (`_PM/Agent-PM/Plans/`):** Every time a new implementation phase is discussed, create a versioned plan document (e.g., `Phase1_Auth_v1.0.md`). Include the objective, scope, and technical prerequisites. If a plan changes, do not overwrite the old one; create `v1.1.md` and explicitly state what changed in the changelog section.
- **Tasks (`_PM/Agent-PM/Tasks/`):** Break down Plans into actionable, versioned task files (e.g., `Task_Setup_Postgres_v1.0.md`). Tasks MUST use Markdown checklists (`- [ ]`). As you complete steps in the codebase, you must return to the active Task file and update the state to checked (`- [x]`).
- **Tests (`_PM/Agent-PM/Tests/`):** Before writing complex logic, define the test parameters in a versioned test file (e.g., `Test_i18n_Routing_v1.0.md`). Include expected inputs and outputs using checklists. Once the actual code passes the test, check off the item (`- [x]`).
- **QA (`_PM/Agent-PM/QA/`):** Use this folder for post-implementation quality assurance checklists (e.g., `QA_Mobile_Responsiveness_v1.0.md`). Check off items only after verifying no regressions exist in the primary application.
- **Documentation (`_PM/Agent-PM/Docs/`):** For every major architecture choice, create an Architectural Decision Record in `/ADRs/`. Maintain living documentation in `/Guides/` for local environment setup, deployments, etc.
