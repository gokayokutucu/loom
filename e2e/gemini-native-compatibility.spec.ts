...
// - Provider calls use a local fake Gemini-compatible HTTP server, not TypeScript-local mode or external APIs.
// - Test data is created through product composer/retry flows and cleaned up by the harness.
import { expect, type Page, test } from "@playwright/test";
import {
  startFakeGeminiServer,
  type FakeGeminiServer,
} from "./helpers/fakeGeminiServer";
...
async function startGeminiNativeScenario(scenarioName: "normal" | "long-running" | "auth-error" | "malformed" = "normal") {
  const fakeProvider = await startFakeGeminiServer(scenarioName);
  
  // Set the environment variables for the spawned loom-service process
  process.env.LOOM_SERVICE_E2E_PROVIDER_PROFILE = "gemini-native";
  process.env.LOOM_GEMINI_BASE_URL = fakeProvider.baseUrl;
  process.env.LOOM_GEMINI_MODEL = "gemini-1.5-flash";
  process.env.LOOM_GEMINI_API_KEY = "gemini-fake-secret-e2e";
...
async function cleanupScenario(input: {
  ...
  delete process.env.LOOM_GEMINI_BASE_URL;
  delete process.env.LOOM_GEMINI_MODEL;
  delete process.env.LOOM_GEMINI_API_KEY;
...
async function openStrictRustApp(page: Page, appUrl: string) {
  ...
  // Pre-load settings with gemini-native enabled
    ...
    ({ key }) => {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          profiles: {
            quickModelId: "gemini-1.5-flash",
            mainModelId: "gemini-1.5-flash",
            mainProviderProfileId: "gemini-native",
            mainProviderDisplayName: "Gemini Native",
            mainProviderKind: "gemini",
          },
...
test.describe("[product-service-backed] Gemini Native Contract Compatibility E2E", () => {
  test("Scenario A & B: Basic generation and streaming", async ({ page }) => {
    test.setTimeout(120_000);
    const scenario = await startGeminiNativeScenario("normal");
    ...
    try {
      ...
      const prompt = `Gemini Native E2E streaming proof ${Date.now()}`;
      await fillPrompt(page, prompt);
      await page.getByRole("button", { name: "Send" }).click();

      // Verify answer streams and displays
      await expect(page.locator(".assistant-message").last()).toContainText(
        "Gemini native visible stream persisted.",
        { timeout: 30_000 }
      );
      await expect(page.locator(".assistant-message").last()).toContainText(
        "Final visible Gemini delta.",
        { timeout: 30_000 }
      );
      ...
      // Verify mock server received the expected Gemini-formatted request
      ...
      expect(assistant?.content).toContain("Gemini native visible stream persisted.");
  ...
  test("Scenario C: Stop cancellation", async ({ page }) => {
    test.setTimeout(120_000);
    const scenario = await startGeminiNativeScenario("long-running");

    try {
      await openStrictRustApp(page, scenario.serviceScenario.appUrl!);

      await fillPrompt(page, `Gemini Native E2E cancellation proof ${Date.now()}`);
      await page.getByRole("button", { name: "Send" }).click();
  ...
  test("Scenario D, E, F: Retry, Regenerate and Settings selection persistence", async ({ page }) => {
    ...
    try {
      ...
      const prompt = `Gemini Native retry regenerate proof ${Date.now()}`;
      await fillPrompt(page, prompt);
      await page.getByRole("button", { name: "Send" }).click();
      await expect(page.locator(".assistant-message").last()).toContainText(
        "Gemini native visible stream persisted.",
      ...
      const geminiCard = page
        ...
        .filter({ hasText: "Gemini Native" });
      await expect(geminiCard).toHaveClass(/enabled/);
      await expect(geminiCard).toContainText("Saved");
      await expect(geminiCard).toContainText("Selected (gemini-1.5-flash)");
    } finally {
      await cleanupScenario(scenario);
    }
  });
  ...
  test("Scenario G: Error mapping and config validation", async ({ page }) => {
    test.setTimeout(120_000);
    const scenario = await startGeminiNativeScenario("auth-error");

    try {
      await openStrictRustApp(page, scenario.serviceScenario.appUrl!);

      await fillPrompt(page, `Gemini Native E2E error mapping proof ${Date.now()}`);
      await page.getByRole("button", { name: "Send" }).click();
      ...
      await expect(page.locator("body")).not.toContainText("gemini-fake-secret-e2e");
