...
## Technical Scope
- **Prop Removal**: Change `onReturnToOrigin={returnToOrigin}` to `onReturnToOrigin={undefined}` (or remove it) in the `<ChatTranscript>` component rendering for the `origin-split-panel` (left side) inside [src/App.tsx](../../src/App.tsx#L14339).
